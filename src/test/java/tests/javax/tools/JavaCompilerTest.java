...
/**
 * TODO verify that we get compiler error messages
 * @author jjYBdx4IL
 */
public class JavaCompilerTest {
    ...
    public void test() throws Exception {
        ...
        StandardJavaFileManager jfm = compiler.getStandardFileManager(diagnostics, Locale.ROOT,
        ...
        Iterable<? extends JavaFileObject> compilationUnit = jfm
                .getJavaFileObjects(srcFileNames.toArray(new String[] {}));
        ...
        JavaCompiler.CompilationTask task = compiler.getTask(null, jfm, diagnostics, optionList, null, compilationUnit);

        assertTrue(task.call());
        duration1 += System.currentTimeMillis();

        for (int i = 0; i < 8; i++) {
            task = compiler.getTask(null, jfm, diagnostics, optionList, null, compilationUnit);
        ...
        for (Diagnostic<? extends JavaFileObject> diagnostic : diagnostics.getDiagnostics()) {
            LOG.error("Error on line {} in {}: {}", diagnostic.getLineNumber(), diagnostic.getSource().toUri(),
