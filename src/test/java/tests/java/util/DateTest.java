...
public class DateTest {
    ...
    public void testCalendarTimeZoneAndClear() {
        ...
        // the set/get date-part related methods are always relative to the
        // calendar's timezone:
    ...
    /**
     * This test demonstrates that the default timezone does *not* matter when
     * creating a Date object from UTC millis.
     */
    @Test
    ...
    public void testInstant() {
        TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
        Date d = new Date(80000 * 1000L);
        
        // start of day
        assertEquals("1970-01-01T00:00:00Z",
            d.toInstant().truncatedTo(ChronoUnit.DAYS).toString());
        // start of next day
        assertEquals("1970-01-02T00:00:00Z",
            d.toInstant().truncatedTo(ChronoUnit.DAYS).plus(1, ChronoUnit.DAYS).toString());
        
        TimeZone.setDefault(TimeZone.getTimeZone("Europe/Berlin"));
        Date d2 = new Date(80000 * 1000L);
        
        // start of day
        assertEquals("1970-01-01T00:00:00Z",
            d2.toInstant().truncatedTo(ChronoUnit.DAYS).toString());
        // start of next day
        assertEquals("1970-01-02T00:00:00Z",
            d2.toInstant().truncatedTo(ChronoUnit.DAYS).plus(1, ChronoUnit.DAYS).toString());
    }
    
    @Test
    public void testInstantGetHour() {
        TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
        Instant d = new Date(0).toInstant();
        // no hour concept without timezone!
        OffsetDateTime odt = d.atOffset(ZoneOffset.UTC);
        assertEquals(0, odt.get(ChronoField.HOUR_OF_DAY));
    }    
}
